...
class aClient(discord.Client):
  ...
  async def update_embed(self):
    ...
    embed = discord.Embed(
        title="Przerwy CeZ",
        description=
        f"Kliknij prycisk by zapisać się do kolejki {self.kolejka_img} na długą przerwę, lub jeśli idziesz na krótką {self.krotka_img} lub długą {self.dluga_img} przerwę.\nNie zapomnij kliknąć wróciłem/am {self.wroc_img} po powrocie",
...
class Break_Ticket(View):
  ...
  async def krotka_btn(self, interaction: discord.Interaction,
    ...
    if interaction.user.mention in client.dluga or interaction.user.mention in client.krotka:
      ...
      await client.update_embed()
      await interaction.edit_original_response(
          content=f"Możesz iść na krótką {client.krotka_img} przerwę")
...
async def self(interaction: discord.Interaction):
  if client.przerwa_msg == None:
    embed = discord.Embed(
        title="Przerwy CeZ",
        description=
        f"Kliknij prycisk by zapisać się do kolejki {client.kolejka_img} na długą przerwę, lub jeśli idziesz na krótką {client.krotka_img} lub długą {client.dluga_img} przerwę.\nNie zapomnij kliknąć wróciłem/am {client.wroc_img} po powrocie",
        color=discord.Colour.from_str("#02f5f5"))
...
@tree.command(name='wklej', description='Wkleja skopiowane zapisane osoby')
async def self(interaction: discord.Interaction):
  await interaction.response.defer(thinking=True, ephemeral=True)
